(*Generated by Sail from cheri128.*)
...
Definition mips_sign_extend {n : Z} (m : Z) (v : mword n) `{ArithFact (m >= n)} 
: mword m :=
   
   sign_extend v m.

Definition mips_zero_extend {n : Z} (m : Z) (v : mword n) `{ArithFact (m >= n)} 
: mword m :=
...
Definition mask {m : Z} (n : Z) (bs : mword m) `{ArithFact (m >= n /\ n > 0)} 
: mword n :=
...
: Exception :=
   ...
   let l__89 := arg_ in
   if sumbool_of_bool ((Z.eqb l__89 0)) then Interrupt
   else if sumbool_of_bool ((Z.eqb l__89 1)) then TLBMod
   else if sumbool_of_bool ((Z.eqb l__89 2)) then TLBL
   else if sumbool_of_bool ((Z.eqb l__89 3)) then TLBS
   else if sumbool_of_bool ((Z.eqb l__89 4)) then AdEL
   else if sumbool_of_bool ((Z.eqb l__89 5)) then AdES
   else if sumbool_of_bool ((Z.eqb l__89 6)) then Sys
   else if sumbool_of_bool ((Z.eqb l__89 7)) then Bp
   else if sumbool_of_bool ((Z.eqb l__89 8)) then ResI
   else if sumbool_of_bool ((Z.eqb l__89 9)) then CpU
   else if sumbool_of_bool ((Z.eqb l__89 10)) then Ov
   else if sumbool_of_bool ((Z.eqb l__89 11)) then Tr
   else if sumbool_of_bool ((Z.eqb l__89 12)) then C2E
   else if sumbool_of_bool ((Z.eqb l__89 13)) then C2Trap
   else if sumbool_of_bool ((Z.eqb l__89 14)) then XTLBRefillL
   else if sumbool_of_bool ((Z.eqb l__89 15)) then XTLBRefillS
   else if sumbool_of_bool ((Z.eqb l__89 16)) then XTLBInvL
   else if sumbool_of_bool ((Z.eqb l__89 17)) then XTLBInvS
   else MCheck.
...
Definition traceException (ex : Exception) 
: unit :=
...
Definition exceptionVectorOffset (ex : Exception) 
: M (mword 12) :=
   
   read_reg CP0Status_ref >>= fun w__0 : StatusReg =>
   returnm ((if (((bits_to_bool (_get_StatusReg_EXL w__0))  : bool)) then
             ...
             else (vec_of_bits [B0;B0;B0;B1;B1;B0;B0;B0;B0;B0;B0;B0]  : mword 12))
    : mword 12).

Definition exceptionVectorBase '(tt : unit) 
: M (mword 64) :=
...
Definition updateBadInstr '(tt : unit) 
: M (unit) :=
...
({rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 64 - 1))} * {rangevar : Z & ArithFact (0 <=
  ...
  rangevar <= (2 ^ 65))}) :=
   ...
   let R3 := sub_vec B3 (vec_of_bits [B0;B0;B1]  : mword 3) in
   ...
   let base : bits 65 :=
     vector_truncate
       (concat_vec (add_vec_int a_top correction_base) (concat_vec c.(Capability_B) e_zeros)) 65 in
   let top : bits 65 :=
     vector_truncate
       (concat_vec (add_vec_int a_top correction_top) (concat_vec c.(Capability_T) e_zeros)) 65 in
   let top : mword 65 :=
     if ((eq_bit (access_vec_dec base 64) B1)) then
       update_vec_dec top 64
         (if sumbool_of_bool ((andb (Z.eqb aHi 1) (Z.eqb tHi 1))) then B1
          else B0)
     else top in
   (build_ex
   (projT1
...
: {rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 64 - 1))} :=
   ...
   build_ex(let '(existT _ base _, existT _ _ _) := getCapBounds c in
   base).

Definition getCapOffset (c : Capability) 
: {rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 64 - 1))} :=
   
   build_ex(let base := projT1 (getCapBase c) in
   projT1
   (emod_with_eq (Z.sub (projT1 (uint c.(Capability_address))) base) (projT1 (pow2 64)))).

Definition capBoundsEqual (c1 : Capability) (c2 : Capability) 
: bool :=
...
: M (o) :=
   ...
   (if ((negb (bits_to_bool (_get_StatusReg_EXL w__0)))) then
      ...
      assert_exp' representable "cheri_prelude_common.sail 325:24 - 325:25" >>= fun _ =>
      read_reg PCC_ref >>= fun w__5 : Capability =>
      (if (w__5.(Capability_sealed)) then
         read_reg PCC_ref >>= fun w__6 : Capability =>
         (assert_exp (Z.eqb (projT1 (getCapOffset w__6)) (projT1 (uint epc))) "cheri_prelude_common.sail 328:47 - 328:48")
          : M (unit)
       else returnm (tt  : unit)) >>
   ...
   read_reg KCC_ref >>= fun w__7 : Capability =>
   let kccBase := projT1 (getCapBase w__7) in
   write_reg
     NextPC_ref
     (sub_vec (add_vec vectorBase (mips_zero_extend 64 vectorOffset)) (to_bits 64 kccBase)) >>
   read_reg KCC_ref >>= fun w__8 : Capability =>
   (set_next_pcc w__8) >>
   (_set_CauseReg_ExcCode CP0Cause_ref (ExceptionCode ex)) >>
   (_set_StatusReg_EXL CP0Status_ref (vec_of_bits [B1]  : mword 1)) >> throw (ISAException (tt)).
...
: MemAccessType :=
   ...
   let l__87 := arg_ in
   if sumbool_of_bool ((Z.eqb l__87 0)) then Instruction
   else if sumbool_of_bool ((Z.eqb l__87 1)) then LoadData
   else StoreData.
...
: AccessLevel :=
   ...
   let l__85 := arg_ in
   if sumbool_of_bool ((Z.eqb l__85 0)) then User
   else if sumbool_of_bool ((Z.eqb l__85 1)) then Supervisor
   else Kernel.
   ...
   (or_boolM
      ...
      (read_reg CP0Status_ref >>= fun w__1 : StatusReg =>
       returnm ((bits_to_bool (_get_StatusReg_ERL w__1))
        : bool))) >>= fun w__2 : bool =>
   (if sumbool_of_bool (w__2) then returnm (Kernel  : AccessLevel)
    else
      read_reg CP0Status_ref >>= fun w__3 : StatusReg =>
      let p__107 := _get_StatusReg_KSU w__3 in
      let b__0 := p__107 in
...
Definition checkCP0AccessHook '(tt : unit) 
: M (unit) :=
...
: M (unit) :=
   ...
   (and_boolM (returnm ((generic_neq accessLevel Kernel)  : bool))
      (read_reg CP0Status_ref >>= fun w__0 : StatusReg =>
       returnm ((negb (bit_to_bool (access_vec_dec (_get_StatusReg_CU w__0) 0)))
        : bool))) >>= fun w__1 : bool =>
...
: M (unit) :=
   ...
   (if ((eq_vec w__0 w__1)) then returnm (TLBIndexMax  : mword 6)
    else
...
: decode_failure :=
   ...
   let l__82 := arg_ in
   if sumbool_of_bool ((Z.eqb l__82 0)) then no_matching_pattern
   else if sumbool_of_bool ((Z.eqb l__82 1)) then unsupported_instruction
   else if sumbool_of_bool ((Z.eqb l__82 2)) then illegal_instruction
   else internal_error.
...
: Comparison :=
   ...
   let l__75 := arg_ in
   if sumbool_of_bool ((Z.eqb l__75 0)) then EQ'
   else if sumbool_of_bool ((Z.eqb l__75 1)) then NE
   else if sumbool_of_bool ((Z.eqb l__75 2)) then GE
   else if sumbool_of_bool ((Z.eqb l__75 3)) then GEU
   else if sumbool_of_bool ((Z.eqb l__75 4)) then GT'
   else if sumbool_of_bool ((Z.eqb l__75 5)) then LE
   else if sumbool_of_bool ((Z.eqb l__75 6)) then LT'
   else LTU.
...
: WordType :=
   ...
   let l__72 := arg_ in
   if sumbool_of_bool ((Z.eqb l__72 0)) then B
   else if sumbool_of_bool ((Z.eqb l__72 1)) then H
   else if sumbool_of_bool ((Z.eqb l__72 2)) then W
   else D.
...
: WordTypeUnaligned :=
   ...
   let l__69 := arg_ in
   if sumbool_of_bool ((Z.eqb l__69 0)) then WL
   else if sumbool_of_bool ((Z.eqb l__69 1)) then WR
   else if sumbool_of_bool ((Z.eqb l__69 2)) then DL
   else DR.
...
: mword 64 :=
   ...
   if sumbool_of_bool (sign) then mips_sign_extend 64 memResult
...
: M ((mword 64 * bool)) :=
   ...
   (if ((eq_vec b__0 (vec_of_bits [B1;B1]  : mword (63 - 62 + 1)))) then
      ...
      returnm (Supervisor, None
       : option (bits 64))
    else if ((eq_vec b__0 (vec_of_bits [B0;B0]  : mword (63 - 62 + 1)))) then
      returnm (User, None
       : option (bits 64))
    else
      assert_exp' false "Pattern match failure at ../mips/mips_tlb.sail 111:66 - 122:5" >>= fun _ =>
      exit tt) >>= fun '((requiredLevel, addr)
...
: CPtrCmpOp :=
   ...
   let l__62 := arg_ in
   if sumbool_of_bool ((Z.eqb l__62 0)) then CEQ
   else if sumbool_of_bool ((Z.eqb l__62 1)) then CNE
   else if sumbool_of_bool ((Z.eqb l__62 2)) then CLT
   else if sumbool_of_bool ((Z.eqb l__62 3)) then CLE
   else if sumbool_of_bool ((Z.eqb l__62 4)) then CLTU
   else if sumbool_of_bool ((Z.eqb l__62 5)) then CLEU
   else if sumbool_of_bool ((Z.eqb l__62 6)) then CEXEQ
   else CNEXEQ.
...
: ClearRegSet :=
   ...
   let l__59 := arg_ in
   if sumbool_of_bool ((Z.eqb l__59 0)) then GPLo
   else if sumbool_of_bool ((Z.eqb l__59 1)) then GPHi
   else if sumbool_of_bool ((Z.eqb l__59 2)) then CLo
   else CHi.
...
Definition max_otype := projT1 (MAX 18).
...
Definition capBitsToCapability (t : bool) (c : mword 128) 
: Capability :=
   ...
   let '(Bs, E, T, lenMSBs) :=
     (if sumbool_of_bool (internal_exponent) then
        ...
        let lenMSBs : bits 2 := (vec_of_bits [B0;B1]  : mword 2) in
        let T : bits 12 := concat_vec (subrange_vec_dec c 89 81) (vec_of_bits [B0;B0;B0]  : mword 3) in
        let Bs : bits 14 :=
          concat_vec (subrange_vec_dec c 77 67) (vec_of_bits [B0;B0;B0]  : mword 3) in
        (Bs, E, T, lenMSBs)
      else
        let lenMSBs : bits 2 := (vec_of_bits [B0;B0]  : mword 2) in
        let T : bits 12 := subrange_vec_dec c 89 78 in
        let Bs : bits 14 := subrange_vec_dec c 77 64 in
        (Bs, E, T, lenMSBs))
      : (mword 14 * mword 6 * mword 12 * mword 2) in
   let carry_out :=
     if ((zopz0zI_u T (subrange_vec_dec Bs 11 0))) then (vec_of_bits [B0;B1]  : mword 2)
     else (vec_of_bits [B0;B0]  : mword 2) in
...
Definition memBitsToCapability (tag : bool) (b : mword 128) 
: Capability :=
...
: mword 31 :=
   ...
   concat_vec (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0]  : mword 12)
     (concat_vec cap.(Capability_uperms) perms).
...
Definition sealCap (cap : Capability) (otyp : mword 24) 
: (bool * Capability) :=
...
: {rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 65))} :=
   ...
   build_ex(let '(existT _ _ _, existT _ top _) := getCapBounds c in
   top).
...
: M ({rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 65))}) :=
   ...
   assert_exp (orb (negb c.(Capability_tag)) (Z.geb top base)) "cheri_prelude_128.sail 319:40 - 319:41" >>
   returnm (build_ex
   (projT1
   (build_ex
   (projT1
    (emod_with_eq (Z.sub top base) (projT1 (pow2 65))))
    : {rangevar : Z & ArithFact (0 <= rangevar /\ rangevar <= (2 ^ 65))}))).
...
: (bool * Capability) :=
   ...
   let base65 := concat_vec (vec_of_bits [B0]  : mword 1) base in
   ...
   let '(Bbits, Tbits, incE, lostSignificantBase, lostSignificantTop) :=
     (if sumbool_of_bool (ie) then
        ...
        let Bbits : mword 14 := concat_vec B_ie (vec_of_bits [B0;B0;B0]  : mword 3) in
        let Tbits : mword 14 := concat_vec T_ie (vec_of_bits [B0;B0;B0]  : mword 3) in
...
Definition getRepresentableAlignmentMask (len : mword 64) 
: mword 64 :=
...
Definition getRepresentableLength (len : mword 64) 
: mword 64 :=
...
: M (string) :=
   ...
   let otype64 : bits 64 :=
     if (cap.(Capability_sealed)) then mips_zero_extend 64 cap.(Capability_otype)
     else ones_implicit 64 tt in
...
: CapEx :=
   ...
   let l__36 := arg_ in
   if sumbool_of_bool ((Z.eqb l__36 0)) then CapEx_None
   else if sumbool_of_bool ((Z.eqb l__36 1)) then CapEx_LengthViolation
   else if sumbool_of_bool ((Z.eqb l__36 2)) then CapEx_TagViolation
   else if sumbool_of_bool ((Z.eqb l__36 3)) then CapEx_SealViolation
   else if sumbool_of_bool ((Z.eqb l__36 4)) then CapEx_TypeViolation
   else if sumbool_of_bool ((Z.eqb l__36 5)) then CapEx_CallTrap
   else if sumbool_of_bool ((Z.eqb l__36 6)) then CapEx_ReturnTrap
   else if sumbool_of_bool ((Z.eqb l__36 7)) then CapEx_TSSUnderFlow
   else if sumbool_of_bool ((Z.eqb l__36 8)) then CapEx_UserDefViolation
   else if sumbool_of_bool ((Z.eqb l__36 9)) then CapEx_TLBNoStoreCap
   else if sumbool_of_bool ((Z.eqb l__36 10)) then CapEx_InexactBounds
   else if sumbool_of_bool ((Z.eqb l__36 11)) then CapEx_GlobalViolation
   else if sumbool_of_bool ((Z.eqb l__36 12)) then CapEx_PermitExecuteViolation
   else if sumbool_of_bool ((Z.eqb l__36 13)) then CapEx_PermitLoadViolation
   else if sumbool_of_bool ((Z.eqb l__36 14)) then CapEx_PermitStoreViolation
   else if sumbool_of_bool ((Z.eqb l__36 15)) then CapEx_PermitLoadCapViolation
   else if sumbool_of_bool ((Z.eqb l__36 16)) then CapEx_PermitStoreCapViolation
   else if sumbool_of_bool ((Z.eqb l__36 17)) then CapEx_PermitStoreLocalCapViolation
   else if sumbool_of_bool ((Z.eqb l__36 18)) then CapEx_PermitSealViolation
   else if sumbool_of_bool ((Z.eqb l__36 19)) then CapEx_AccessSystemRegsViolation
   else if sumbool_of_bool ((Z.eqb l__36 20)) then CapEx_PermitCCallViolation
   else if sumbool_of_bool ((Z.eqb l__36 21)) then CapEx_AccessCCallIDCViolation
   else if sumbool_of_bool ((Z.eqb l__36 22)) then CapEx_PermitUnsealViolation
...
: M (unit) :=
   ...
   (if ((Bool.eqb (bits_to_bool (_get_StatusReg_ERL w__0)) (bit_to_bool B1))) then
      read_reg ErrorEPCC_ref >>= fun w__1 : Capability => (set_next_pcc w__1)  : M (unit)
    else read_reg EPCC_ref >>= fun w__2 : Capability => (set_next_pcc w__2)  : M (unit))
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec addr
      ...
      assert_exp (eq_vec (and_vec addr cap_addr_mask)
                    (and_vec (add_vec addr (to_bits 64 (Z.sub size 1))) cap_addr_mask)) "cheri_prelude_common.sail 461:85 - 461:86" >>
...
: M (bool) :=
   ...
   assert_exp (eq_vec (and_vec addr cap_addr_mask)
                 (and_vec (add_vec addr (to_bits 64 (Z.sub size 1))) cap_addr_mask)) "cheri_prelude_common.sail 473:85 - 473:86" >>
...
: M (unit) :=
   ...
   (match accessType with
    | Instruction =>
       assert_exp' false "cheri_prelude_common.sail 486:34 - 486:35" >>= fun _ => exit tt
    | LoadData =>
...
Definition set_CP0EPC (newEPC : mword 64) 
: M (unit) :=
...
Definition get_CP0ErrorEPC '(tt : unit) 
: M (mword 64) :=
...
Definition set_CP0ErrorEPC (v : mword 64) 
: M (unit) :=
...
Definition getCapFlags (cap : Capability)  : mword 1 :=  (vec_of_bits [B0]  : mword 1).
       ...
       (CGetAddr
          ((rd, cb)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 10 0)
                  (vec_of_bits [B1;B0;B0;B1;B0;B1;B1;B1;B1;B1;B1]  : mword (10 - 0 + 1))))) then
     let rd : regno := subrange_vec_dec v__0 20 16 in
     let cb : regno := subrange_vec_dec v__0 15 11 in
     Some
       (CGetFlags
          ((rd, cb)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 10 0)
                  (vec_of_bits [B1;B0;B0;B0;B0;B1;B1;B1;B1;B1;B1]  : mword (10 - 0 + 1))))) then
     let rt : regno := subrange_vec_dec v__0 20 16 in
     let rs : regno := subrange_vec_dec v__0 15 11 in
     Some
       (CRAP
          ((rt, rs)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 10 0)
                  (vec_of_bits [B1;B0;B0;B0;B1;B1;B1;B1;B1;B1;B1]  : mword (10 - 0 + 1))))) then
     let rt : regno := subrange_vec_dec v__0 20 16 in
     let rs : regno := subrange_vec_dec v__0 15 11 in
     Some
       (CRAM
       ...
       (CSetBoundsExact
          ((cd, cs, rt)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 5 0)
                  (vec_of_bits [B0;B0;B1;B1;B1;B0]  : mword (5 - 0 + 1))))) then
     let rt : regno := subrange_vec_dec v__0 10 6 in
     let cs : regno := subrange_vec_dec v__0 15 11 in
     let cd : regno := subrange_vec_dec v__0 20 16 in
     Some
       ...
       (CMOVX
          ((cd, cs, rs, true)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 5 0)
                  (vec_of_bits [B1;B0;B0;B0;B1;B0]  : mword (5 - 0 + 1))))) then
     let rs : regno := subrange_vec_dec v__0 10 6 in
     let cs : regno := subrange_vec_dec v__0 15 11 in
     let cd : regno := subrange_vec_dec v__0 20 16 in
     Some
       (CSetAddr
          ((cd, cs, rs)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 5 0)
                  (vec_of_bits [B1;B0;B0;B0;B1;B1]  : mword (5 - 0 + 1))))) then
     let rs : regno := subrange_vec_dec v__0 10 6 in
     let rd : regno := subrange_vec_dec v__0 20 16 in
     let cs : regno := subrange_vec_dec v__0 15 11 in
     Some
       (CGetAndAddr
          ((rd, cs, rs)))
               ...
               (eq_vec (subrange_vec_dec v__0 31 21)
                  (vec_of_bits [B0;B1;B0;B0;B1;B0;B0;B0;B0;B0;B0]  : mword (31 - 21 + 1)))
               (eq_vec (subrange_vec_dec v__0 5 0)
                  (vec_of_bits [B1;B0;B0;B1;B0;B0]  : mword (5 - 0 + 1))))) then
     let rs : regno := subrange_vec_dec v__0 10 6 in
     let cs : regno := subrange_vec_dec v__0 15 11 in
     let cd : regno := subrange_vec_dec v__0 20 16 in
     Some
    ...
    if sumbool_of_bool (conditional) then
      ...
      (if (((bit_to_bool (access_vec_dec w__1 0))  : bool)) then
         (match width with
          ...
          : M (bool)
       else returnm (projT1 (build_ex false  : {_bool : bool & ArithFact (not (_bool = true))}))) >>= fun success : bool =>
...
: M (unit) :=
   ...
   (if ((eq_vec b__12 (vec_of_bits [B0;B0]  : mword (1 - 0 + 1)))) then
      ...
      (MEMw_wrapper wordAddr 3 (subrange_vec_dec reg_val 23 0))
       : M (unit)
    else if ((eq_vec b__12 (vec_of_bits [B1;B1]  : mword (1 - 0 + 1)))) then
      (MEMw_wrapper wordAddr 4 (subrange_vec_dec reg_val 31 0))
       : M (unit)
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1277:12 - 1283:15" >>= fun _ =>
      exit tt)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__8 (vec_of_bits [B0;B0]  : mword (1 - 0 + 1)))) then
      ...
      (MEMw_wrapper pAddr 2 (subrange_vec_dec reg_val 31 16))
       : M (unit)
    else if ((eq_vec b__8 (vec_of_bits [B1;B1]  : mword (1 - 0 + 1)))) then
      (MEMw_wrapper pAddr 1 (subrange_vec_dec reg_val 31 24))
       : M (unit)
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1257:12 - 1263:13" >>= fun _ =>
      exit tt)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((orb (NotWordVal opA) (NotWordVal opB))) then
      ...
      (if ((neq_bool (bit_to_bool (access_vec_dec temp33 32))
              (bit_to_bool (access_vec_dec temp33 31)))) then
         (SignalException Ov)
          : M (unit)
...
: M (unit) :=
   ...
   (if ((eq_vec b__40 (vec_of_bits [B0;B0;B0]  : mword (2 - 0 + 1)))) then
      ...
      (MEMw_wrapper wordAddr 7 (subrange_vec_dec reg_val 55 0))
       : M (unit)
    else if ((eq_vec b__40 (vec_of_bits [B1;B1;B1]  : mword (2 - 0 + 1)))) then
      (MEMw_wrapper wordAddr 8 (subrange_vec_dec reg_val 63 0))
       : M (unit)
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1373:12 - 1383:13" >>= fun _ =>
      exit tt)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__32 (vec_of_bits [B0;B0;B0]  : mword (2 - 0 + 1)))) then
      ...
      (MEMw_wrapper pAddr 2 (subrange_vec_dec reg_val 63 48))
       : M (unit)
    else if ((eq_vec b__32 (vec_of_bits [B1;B1;B1]  : mword (2 - 0 + 1)))) then
      (MEMw_wrapper pAddr 1 (subrange_vec_dec reg_val 63 56))
       : M (unit)
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1347:12 - 1357:13" >>= fun _ =>
      exit tt)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__4 (vec_of_bits [B0;B0]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 31 8) (subrange_vec_dec mem_val 31 24))
       : mword (31 - 8 + 1 + (31 - 24 + 1)))
    else if ((eq_vec b__4 (vec_of_bits [B0;B1]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 31 16) (subrange_vec_dec mem_val 31 16))
       : mword (31 - 16 + 1 + (31 - 16 + 1)))
    else if ((eq_vec b__4 (vec_of_bits [B1;B0]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 31 24) (subrange_vec_dec mem_val 31 8))
       : mword (31 - 24 + 1 + (31 - 8 + 1)))
    else if ((eq_vec b__4 (vec_of_bits [B1;B1]  : mword (1 - 0 + 1)))) then
      returnm (mem_val
       : mword (8 * 4))
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1236:32 - 1242:15" >>= fun _ =>
      exit tt) >>= fun result : bits 32 =>
...
: M (unit) :=
   ...
   (if ((eq_vec b__0 (vec_of_bits [B0;B0]  : mword (1 - 0 + 1)))) then
      returnm (mem_val
       : mword (8 * 4))
    else if ((eq_vec b__0 (vec_of_bits [B0;B1]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 23 0) (subrange_vec_dec reg_val 7 0))
       : mword (23 - 0 + 1 + (7 - 0 + 1)))
    else if ((eq_vec b__0 (vec_of_bits [B1;B0]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 15 0) (subrange_vec_dec reg_val 15 0))
       : mword (15 - 0 + 1 + (15 - 0 + 1)))
    else if ((eq_vec b__0 (vec_of_bits [B1;B1]  : mword (1 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 7 0) (subrange_vec_dec reg_val 23 0))
       : mword (7 - 0 + 1 + (23 - 0 + 1)))
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1216:32 - 1222:8" >>= fun _ =>
      exit tt) >>= fun result : bits 32 =>
   (wGPR rt (mips_sign_extend 64 result))
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__24 (vec_of_bits [B0;B0;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 8) (subrange_vec_dec mem_val 63 56))
       : mword (63 - 8 + 1 + (63 - 56 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B0;B0;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 16) (subrange_vec_dec mem_val 63 48))
       : mword (63 - 16 + 1 + (63 - 48 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B0;B1;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 24) (subrange_vec_dec mem_val 63 40))
       : mword (63 - 24 + 1 + (63 - 40 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B0;B1;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 32) (subrange_vec_dec mem_val 63 32))
       : mword (63 - 32 + 1 + (63 - 32 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B1;B0;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 40) (subrange_vec_dec mem_val 63 24))
       : mword (63 - 40 + 1 + (63 - 24 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B1;B0;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 48) (subrange_vec_dec mem_val 63 16))
       : mword (63 - 48 + 1 + (63 - 16 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B1;B1;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec reg_val 63 56) (subrange_vec_dec mem_val 63 8))
       : mword (63 - 56 + 1 + (63 - 8 + 1)))
    else if ((eq_vec b__24 (vec_of_bits [B1;B1;B1]  : mword (2 - 0 + 1)))) then
      returnm (mem_val
       : mword (8 * 8))
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1323:20 - 1333:15" >>= fun _ =>
      exit tt) >>= fun w__9 : mword 64 =>
   (wGPR rt w__9)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__16 (vec_of_bits [B0;B0;B0]  : mword (2 - 0 + 1)))) then
      returnm (mem_val
       : mword (8 * 8))
    else if ((eq_vec b__16 (vec_of_bits [B0;B0;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 55 0) (subrange_vec_dec reg_val 7 0))
       : mword (55 - 0 + 1 + (7 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B0;B1;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 47 0) (subrange_vec_dec reg_val 15 0))
       : mword (47 - 0 + 1 + (15 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B0;B1;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 39 0) (subrange_vec_dec reg_val 23 0))
       : mword (39 - 0 + 1 + (23 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B1;B0;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 31 0) (subrange_vec_dec reg_val 31 0))
       : mword (31 - 0 + 1 + (31 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B1;B0;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 23 0) (subrange_vec_dec reg_val 39 0))
       : mword (23 - 0 + 1 + (39 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B1;B1;B0]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 15 0) (subrange_vec_dec reg_val 47 0))
       : mword (15 - 0 + 1 + (47 - 0 + 1)))
    else if ((eq_vec b__16 (vec_of_bits [B1;B1;B1]  : mword (2 - 0 + 1)))) then
      returnm ((concat_vec (subrange_vec_dec mem_val 7 0) (subrange_vec_dec reg_val 55 0))
       : mword (7 - 0 + 1 + (55 - 0 + 1)))
    else
      assert_exp' false "Pattern match failure at ../mips/mips_insts.sail 1298:19 - 1308:15" >>= fun _ =>
      exit tt) >>= fun w__9 : mword 64 =>
   (wGPR rt w__9)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((neq_bool (bit_to_bool (access_vec_dec sum65 64)) (bit_to_bool (access_vec_dec sum65 63))))
    then
      (SignalException Ov)
       : M (unit)
...
: M (unit) :=
   ...
   (checkCP2usable tt) >>
   let l__27 := projT1 (uint sel) in
   (if sumbool_of_bool ((Z.eqb l__27 0)) then returnm (false, false)
    else if sumbool_of_bool ((Z.eqb l__27 1)) then returnm (false, false)
    else if sumbool_of_bool ((Z.eqb l__27 8)) then returnm (false, true)
    else if sumbool_of_bool ((Z.eqb l__27 22)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__27 23)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__27 28)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__27 29)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__27 30)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__27 31)) then returnm (true, true)
    else (SignalException ResI)  : M ((bool * bool))) >>= fun '((needSup, needAccessSys)
   : (bool * bool)) =>
   (and_boolMP
      ((returnm (build_ex
        needAccessSys)) : M ({_bool : bool & ArithFact (iff (_bool = true) (needAccessSys = true))}))
      (build_trivial_ex
      ((pcc_access_system_regs tt) >>= fun w__9 : bool => returnm ((negb w__9)  : bool))) : M ({_bool : bool & ArithFact (exists simp_1 , iff (_bool =
     true) (needAccessSys = true /\ simp_1 = true))})) >>= fun '(existT _ w__10 _) =>
   (if sumbool_of_bool (w__10) then
    ...
    (and_boolMP
       ((returnm (build_ex
         needSup)) : M ({_bool : bool & ArithFact (iff (_bool = true) (needSup = true))}))
       (build_trivial_ex
       ((getAccessLevel tt) >>= fun w__11 : AccessLevel =>
        returnm ((negb (grantsAccess w__11 Supervisor))
         : bool))) : M ({_bool : bool & ArithFact (exists simp_1 , iff (_bool = true) (needSup =
      true /\
      simp_1 = true))})) >>= fun '(existT _ w__12 _) =>
    if sumbool_of_bool (w__12) then
      (raise_c2_exception CapEx_AccessSystemRegsViolation sel)
       : M (unit)
    else
      (readCapReg cb) >>= fun capVal =>
      let l__18 := projT1 (uint sel) in
      (if sumbool_of_bool ((Z.eqb l__18 0)) then write_reg DDC_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 1)) then write_reg CULR_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 8)) then write_reg CPLR_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 22)) then write_reg KR1C_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 23)) then write_reg KR2C_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 28)) then write_reg ErrorEPCC_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 29)) then write_reg KCC_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 30)) then write_reg KDC_ref capVal  : M (unit)
       else if sumbool_of_bool ((Z.eqb l__18 31)) then write_reg EPCC_ref capVal  : M (unit)
...
Definition execute_CSetFlags (cd : mword 5) (cb : mword 5) (rt : mword 5) 
: M (unit) :=
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((negb exact)) then
...
Definition execute_CSetAddr (cd : mword 5) (cb : mword 5) (rt : mword 5) 
: M (unit) :=
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((Z.gtb (Z.add vAddr cap_size) (projT1 (getCapTop cb_val)))) then
       ...
       if sumbool_of_bool ((andb cs_val.(Capability_tag) noStoreCap)) then
         (raise_c2_exception CapEx_TLBNoStoreCap cs)
          : M (unit)
       else
         ((read_reg CP0LLBit_ref)  : M (mword 1)) >>= fun w__0 : mword 1 =>
         (if (((bit_to_bool (access_vec_dec w__0 0))  : bool)) then
            (MEMw_tagged_conditional pAddr cap_size cs_val.(Capability_tag) (capToMemBits cs_val))
             : M (bool)
          else returnm (projT1 (build_ex false  : {_bool : bool & ArithFact (not (_bool = true))}))) >>= fun success =>
         (wGPR rd (mips_zero_extend 64 (bool_to_bits success)))
          : M (unit))
       : M (unit))
    : M (unit).
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((Z.gtb (Z.add vAddr cap_size) (projT1 (getCapTop cb_val)))) then
       ...
       if sumbool_of_bool ((andb cs_val.(Capability_tag) noStoreCap)) then
         (raise_c2_exception CapEx_TLBNoStoreCap cs)
          : M (unit)
       else (MEMw_tagged pAddr cap_size cs_val.(Capability_tag) (capToMemBits cs_val))  : M (unit))
       : M (unit))
    : M (unit).
...
: M (unit) :=
   ...
   (checkCP2usable tt) >>
   let l__9 := projT1 (uint sel) in
   (if sumbool_of_bool ((Z.eqb l__9 0)) then returnm (false, false)
    else if sumbool_of_bool ((Z.eqb l__9 1)) then returnm (false, false)
    else if sumbool_of_bool ((Z.eqb l__9 8)) then returnm (false, true)
    else if sumbool_of_bool ((Z.eqb l__9 22)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__9 23)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__9 28)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__9 29)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__9 30)) then returnm (true, true)
    else if sumbool_of_bool ((Z.eqb l__9 31)) then returnm (true, true)
    else (SignalException ResI)  : M ((bool * bool))) >>= fun '((needSup, needAccessSys)
   : (bool * bool)) =>
   (and_boolMP
      ((returnm (build_ex
   ...
   (if sumbool_of_bool (w__10) then
    ...
    if sumbool_of_bool (w__12) then
      ...
      let l__0 := projT1 (uint sel) in
      (if sumbool_of_bool ((Z.eqb l__0 0)) then read_reg DDC_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 1)) then read_reg CULR_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 8)) then read_reg CPLR_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 22)) then read_reg KR1C_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 23)) then read_reg KR2C_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 28)) then read_reg ErrorEPCC_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 29)) then read_reg KCC_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 30)) then read_reg KDC_ref  : M (Capability)
       else if sumbool_of_bool ((Z.eqb l__0 31)) then read_reg EPCC_ref  : M (Capability)
       else assert_exp' false "CReadHwr: should be unreachable code" >>= fun _ => exit tt) >>= fun capVal : Capability =>
      (writeCapReg cd capVal)
       : M (unit))
    : M (unit).

Definition execute_CRAP (rt : mword 5) (rs : mword 5) 
: M (unit) :=
   
   (checkCP2usable tt) >>
   (rGPR rs) >>= fun len => (wGPR rt (getRepresentableLength len))  : M (unit).

Definition execute_CRAM (rt : mword 5) (rs : mword 5) 
: M (unit) :=
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((Z.gtb (Z.add vAddr (Z.mul caps_per_cacheline cap_size))
                              (projT1
                               (getCapTop cb_val)))) then
         (raise_c2_exception CapEx_LengthViolation cb)
          : M (unit)
       else if sumbool_of_bool ((Z.ltb vAddr (projT1 (getCapBase cb_val)))) then
         (raise_c2_exception CapEx_LengthViolation cb)
          : M (unit)
       else if sumbool_of_bool ((negb
                                   (Z.eqb
                                      (projT1
                                       (emod_with_eq vAddr (Z.mul cap_size caps_per_cacheline))) 0)))
       then
         ...
         (foreach_ZM_up loop_i_lower loop_i_upper 1 x
           (fun i _ x =>
             (MEMr_tagged (add_vec_int pAddr (Z.mul i cap_size)) cap_size) >>= fun '(tag, _) =>
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((Z.gtb (Z.add vAddr cap_size) (projT1 (getCapTop cb_val)))) then
         ...
         (MEMr_tagged_reserve pAddr cap_size) >>= fun '((tag, mem)
         : (bool * CapBits)) =>
         let cap :=
           memBitsToCapability
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (if sumbool_of_bool ((Z.gtb (Z.add vAddr cap_size) (projT1 (getCapTop cb_val)))) then
         ...
         (TLBTranslateC vAddr64 LoadData) >>= fun '(pAddr, suppressTag) =>
         (MEMr_tagged pAddr cap_size) >>= fun '(tag, mem) =>
         let cap :=
           memBitsToCapability
             (andb tag (andb cb_val.(Capability_permit_load_cap) (negb suppressTag))) mem in
         (writeCapReg cd cap)
...
Definition execute_CGetFlags (rd : mword 5) (cb : mword 5) 
: M (unit) :=
...
Definition execute_CGetAndAddr (rd : mword 5) (cb : mword 5) (rs : mword 5) 
: M (unit) :=
...
: M (unit) :=
   ...
   (if ((negb cb_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cb)  : M (unit)
      ...
      (writeCapReg cd {[ null_cap with Capability_address := (ones_implicit 64 tt) ]})
       : M (unit))
    : M (unit).
...
: M (unit) :=
   ...
   (if ((eq_vec b__155 (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0]  : mword 11))) then
      ...
      (if ((negb cs_val.(Capability_tag))) then
       ...
       : M (unit)
    else if ((eq_vec b__155 (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B1]  : mword 11))) then
      (checkCP2usable tt) >>
      ((read_reg InBranchDelay_ref)  : M (mword 1)) >>= fun w__1 : mword 1 =>
      ...
      assert_exp' false "Pattern match failure at ../mips/mips_ri.sail 40:16 - 44:1" >>= fun _ =>
      exit tt)
    : M (unit).
...
: M (unit) :=
   ...
   (if ((negb cs_val.(Capability_tag))) then (raise_c2_exception CapEx_TagViolation cs)  : M (unit)
                ...
                ((Z.eqb (projT1 (getCapCursor ct_val)) MAX_U64)
                 : bool))) then
...
Definition execute_CAndAddr (cd : mword 5) (cb : mword 5) (rt : mword 5) 
: M (unit) :=
...
: M (unit) :=
   ...
   (if sumbool_of_bool (link) then
      (wGPR (vec_of_bits [B1;B1;B1;B1;B1]  : mword 5) linkVal)
       : M (unit)
    else returnm (tt  : unit))
    : M (unit).
...
: M (unit) :=
   ...
   (match merge_var with
    ...
    | CLoadLinked ((rd, cb, signext, width)) =>
       (execute_CLoadLinked rd cb signext width)  : M (unit)
...
{| Capability_tag := false; 
   Capability_uperms := (vec_of_bits [B0;B0;B0;B0]  : mword 4); 
   Capability_permit_set_CID := false; 
   Capability_access_system_regs := false; 
   Capability_permit_unseal := false; 
   Capability_permit_ccall := false; 
   ...
   Capability_permit_execute := false; 
   Capability_global := false; 
   Capability_reserved := (vec_of_bits [B0;B0;B0]  : mword 3); 
   Capability_internal_e := false; 
   Capability_E := (vec_of_bits [B0;B0;B0;B0;B0;B0]  : mword 6); 
   Capability_sealed := false; 
   Capability_B := (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0]  : mword 14); 
   Capability_T := (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0]  : mword 14); 
   Capability_otype :=
     (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0]  : mword 18); 
   Capability_address :=
     (vec_of_bits [B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;
                   B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;
                   B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;B0;
